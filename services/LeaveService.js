...
const getLeaveByMonthAndUserId = (leaveData) => {
    return new Promise((resolve, reject) => {
        ...
        LeaveModel.aggregate([
            {
                $match: { 'userId': leaveData.userId, 'date.month': leaveData.month, 'date.year': leaveData.year }
            },
            {
                $sort: { 'date.year': 1, 'date.month': 1, 'date.date': 1 }
            }
...
const getLeavesByYearAndUserId = (leaveData) => {
    return new Promise((resolve, reject) => {
        ...
        LeaveModel.aggregate([
            {
                $match: { 'userId': leaveData.userId, 'date.year': leaveData.year }
            },
            {
                $sort: { 'date.year': 1, 'date.month': 1, 'date.date': 1 }
            }
...
const getTodayNotPresentUsers = (currentDate) => {
    return new Promise((resolve, reject) => {
        ...
        // console.log("currentDate:", currentDate);
        // PurchaseOrder.find({ 'abc': { $in: ["abc"] } }, (purerr, purres) => {
        // LeaveModel.find({ status: 'Approved', totalDate: { $in: ['2019-11-30']   }})
        LeaveModel.aggregate([
            {
                $match: { "status": 'Approved' }
            },
            ...
            if (err) {
                console.log("err:", err)
                reject({ status: 500, message: "Leave not found." });
            } else if (leave && leave.length) {
                console.log("leave:", leave);
                resolve({ status: 200, message: "Leave found.", data: leave });
            } else {
                reject({ status: 404, message: "Leave not found." });
            }
        })
    })
}
