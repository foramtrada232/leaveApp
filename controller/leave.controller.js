	...
	UserModel.findOne({ 'email': req.user.email }).exec((err, user) => {
		if (err) return rea.status(500).json({ message: 'Login User not found.' })
			...
			const leaveData = {
				...
				userId: userId,
				appliedOn: result[2] + '-' + result[1] + '-' + result[0]
			}
			if (req.body.noOfDays) {
				...
				let currentleaveDate = req.body.date.split("T");
				var someDate = new Date(currentleaveDate[0]);
				var array = []
				var n = req.body.noOfDays;
				var newdate = currentleaveDate[0].split('-');
				var currentDate = newdate[2] + '-' + newdate[1] + '-' + newdate[0]
				array.push(currentDate)
				for (i = 1; i < n; i++) {
					someDate.setDate(someDate.getDate() + 1)
					var dateFormated = someDate.toISOString().substr(0, 10);
					var newDate = dateFormated.split('-');
					var finalDate = newDate[2] + '-' + newDate[1] + '-' + newDate[0]
					array.push(finalDate)
					if (someDate.getDay() == 0) {
						array.pop()
						n = n + 1;
						console.log("if calling", someDate.getDay())
					}
				}
				leaveData['totalDate'] = array;
...
updateLeaveByStatus = function (req, res) {
	...
	LeaveService.updateLeaveByStatus(leaveData).then((response) => {
		...
		UserModel.find({ '_id': response.data.userId }).exec((err, user) => {
			...
			if (user) {
				console.log("USER:", user)
				UserModel.findOne({ 'email': req.user.email }).exec((err, users) => {
					if (err) return res.status(500).json({ message: 'Login User not found.' })
					else {
						console.log("final user:", users)
						LeaveModel.findOneAndUpdate({ '_id': response.data._id }, { $set: { 'approvedBy': users.name } }, { upsert: true, new: true }).exec((err, admin) => {
							if (err) {
								console.log("errrrrr", err)
							}
							else if (admin) {
								console.log("bghgjijoikj=======>", admin)
								if (response.data.status == 'Approved') {
									...
									const obj = {
										...
										'notification': {
											...
											body: 'Your leave has been approved by ' + users.name + '.',
										}
									}
									console.log('obj============>', obj)
									NotificationService.sendNotification(obj);
									...
									const obj = {
										...
										'notification': {
											title: 'Leave Notification',
											body: 'Your leave has been rejected by ' + users.name + '.',
										}
									}
									console.log('obj============>', obj)
									NotificationService.sendNotification(obj);
...
crontab.scheduleJob('0 0 1 * *', leaveUpdateByMonthAndyear = () => {
	...
	return new Promise((resolve, reject) => {
		...
		if (leaveData.date[2] != leaveData.year) {
			...
			UserModel.find({ designation: 'Developer' }).then((users) => {
				...
				users.forEach((user) => {
					...
					UserModel.update({ _id: user._id }, { $set: { total_leave: updated } }, { upsert: true, new: true }).exec((err, upUser) => {
						if (err) {
							...
							resolve({ status: 200, message: 'Leave updated.', data: upUser })
						}
					})
				})
 ...
 */
crontab.scheduleJob('0 30 11 * * *', tomorrowNotPresentUserList = () => {
	console.log("hey");
	return new Promise((resolve, reject) => {
		console.log("hey");
		const userId = [];
